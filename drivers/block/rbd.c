 ...
 *
 * For now, there isn't much value to defining symbolic constants
 * to represent these.  rbd_image_format_valid() should suffice.
 */
typedef u32	rbd_image_format_t;
...
struct rbd_device {
	...
	rbd_image_format_t	image_format;
...
static bool rbd_image_format_valid(rbd_image_format_t image_format)
{
...
static int rbd_header_set_mapping(struct rbd_device *rbd_dev, char *snap_name)
{
	int ret;
		       ...
		       size_t count)
{
	...
	rc = rbd_header_set_mapping(rbd_dev, snap_name);
	if (rc)
		goto err_out_header;
